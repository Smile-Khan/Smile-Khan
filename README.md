[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&pause=1000&width=1400&height=80&lines=%F0%9F%91%8B+Hi%2C+I'm+Pathan+;%E2%80%94+a+Full-Stack+Developer+with+a+passion+for+clean+code+and+efficient+systems.)](https://git.io/typing-svg)


🌱 Constant Learner | 💻 Full-Stack Developer in Training | ☕ Java + Spring Boot Enthusiast  
📍 Based in Maharashtra | Open to Relocation   

---

## 🚀 About Me
- 🎓 B.Tech in Computer Science & Engineering  
- 👨‍💻 Backend developer with hands-on experience in Java, Spring Boot, REST APIs  
- 🔍 Learning Full Stack Development @ AccioJob  
- 🧠 Passionate about clean code, design patterns, and scalable systems  
- 🔄 700+ day LeetCode streak | 500+ day GeeksforGeeks streak | 600+ day AccioJob streak  

---

## 🛠️ Tech Stack

```text
💻 Backend      : Java, Spring Boot, REST APIs, Hibernate, JPA  
🗃️ Database     : MySQL, PostgreSQL, MongoDB, H2  
🧰 Tools & Dev  : Git, Postman, Maven, Docker, IntelliJ, Eclipse  
☁️ Cloud/CI-CD  : Azure (App Services, Pipelines), GitHub Actions  
🖼️ Frontend     : HTML, CSS, JavaScript (basic), React (learning)
```

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Smile-Khan&theme=radical" />
  <img src="https://github-readme-stats.vercel.app/api?username=Smile-Khan&show_icons=true&theme=radical" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Smile-Khan&layout=compact&theme=radical" />
</p>

![GitHub Snake Light](https://github.com/Smile-Khan/Smile-Khan/blob/output/github-contribution-grid-snake.svg)
![GitHub Snake Dark](https://github.com/Smile-Khan/Smile-Khan/blob/output/github-contribution-grid-snake-dark.svg?palette=github-dark)
---

## 📂 Featured Projects

- 🔗 [Book API](https://github.com/Smile-Khan/Book-API) – A RESTful API built using Spring Boot & MySQL  
- 🔗 [Todo Manager (Full Stack)](https://github.com/Smile-Khan/todo-app) – React + Spring Boot project to manage tasks  
- 🔗 [SQL Code Series](https://github.com/Smile-Khan/SQL-50-LEETCODE) – Real-world SQL problems with clean queries and use cases  

---

## 📫 Let's Connect

- 🌐 [LinkedIn](https://www.linkedin.com/in/pathan-ismailkhan-b67826222/)
- 🧠 [LeetCode](https://leetcode.com/u/ismailkhan21/)  

---

## 🔄 Currently Improving

- ☕ Advanced Java + Spring Boot microservices  
- 📁 Building projects from scratch  
- 🧠 System design fundamentals  
- 🧪 Writing test cases and learning JUnit & Mockito  
- ✍️ Regularly sharing knowledge through [LinkedIn articles](https://www.linkedin.com/in/pathan-ismailkhan-b67826222/)
